...
def handler():
    ...
    if current_user.is_authenticated:
        match action:
            ...
            case 'create-transaction':#송금하기기
                sender = current_user.id # 내 id
                recipient_name = request.form.get('recipient')#송금 받는사람 name
                if recipient_name:
                    if recipient_name != current_user.username:# 자신이면 안됨됨
                        recipient = User.query.filter_by(username=recipient_name).first()
                        if recipient:
                            amount = int(request.form.get('amount'))# 지정한 양양
                            if amount > 0:
